...
namespace Sudoku
{
    public partial class SudokuForm: Form
    {
        private void PrintDocument()
        {
            ...
            catch(Win32Exception)
            {
                if(printParameters.PrintResult!=0)
                    MessageBox.Show(Resources.NotPrinted+Environment.NewLine+PrintParameters.PrintError(printParameters.PrintResult));
            }
        ...
        private void PrintDialog()
        {
            ...
            {
                MessageBox.Show(Resources.InvalidProblem+Environment.NewLine+Resources.PrintNotPossible, ProductName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        private void PrintBooklet()
        {
            ...
            if(printParameters.Problems.Count<1)
                MessageBox.Show(Resources.NoProblems);
            else
            {
                printSudokuDialog.UseEXDialog=true;
                if(printSudokuDialog.ShowDialog()==DialogResult.OK)
        ...
        private void GenerateProblems4Booklet()
        {
            ...
            if(Settings.Default.AutoSaveBooklet)
            {
                if(!Directory.Exists(Settings.Default.ProblemDirectory))
                {
                    ...
                    {
                        MessageBox.Show(String.Format(cultureInfo, Resources.CreateDirectoryFailed, Settings.Default.ProblemDirectory), ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if(Settings.Default.AutoSaveBooklet)
            {
                ...
                {
                    MessageBox.Show(String.Format(cultureInfo, Resources.CreateDirectoryFailed, generationParameters.BaseDirectory), ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Settings.Default.AutoSaveBooklet=false;
                }
            }
            printParameters=new PrintParameters();
