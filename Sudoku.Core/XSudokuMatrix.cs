...
/// </summary>
[Serializable]
public class XSudokuMatrix: BaseMatrix
{
    /// <summary>
    ...
    {
        UpDiagonal = new BaseCell[SudokuGrid.SudokuSize];
        DownDiagonal = new BaseCell[SudokuGrid.SudokuSize];
        for(int row = 0; row < SudokuGrid.SudokuSize; row++)
            for(int i = 0; i < SudokuGrid.SudokuSize; i++)
            {
                ...
                if(!Cell(row, SudokuGrid.SudokuSize - 1 - row).SameRectangle(Cell(i, SudokuGrid.SudokuSize - 1 - i))) Cell(row, SudokuGrid.SudokuSize - 1 - row).AddNeighbor(ref Matrix[i][SudokuGrid.SudokuSize - 1 - i]);
            }
        for(int i = 0; i < SudokuGrid.SudokuSize; i++)
        {
            DownDiagonal[i] = Cell(i, i);
            UpDiagonal[i] = Cell(i, SudokuGrid.SudokuSize - 1 - i);
        }
    }
    ...
    {
        if(row == col || row + col == SudokuGrid.SudokuSize - 1)
    ...
    public override float SeverityLevel
    {
        get
        {
            if((internalSeverityLevel = base.SeverityLevel) == float.NaN)
                return float.NaN;
            internalSeverityLevel /= 1.1f;
