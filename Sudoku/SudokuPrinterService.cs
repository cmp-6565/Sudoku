...
internal class SudokuPrinterService: IDisposable
{
    ...
    private void PrintProblem(float x, float y, Graphics g)
    {
        ...
        else
        {
            ...
            {
                String problemTitle = (printParameters.Problems.Count > 1 ? String.Format(cultureInfo, Resources.Problem, printParameters.CurrentProblem + 1) + ": " : String.Empty) + currentProblem.SeverityLevelCategory.ToDisplayText() + (settings.PrintInternalSeverity ? " (" + currentProblem.SeverityLevel + ")" : "");
